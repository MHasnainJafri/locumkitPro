                ...
                <div class="qus-tabs">
                    <ul>
                        <li class="{{ $controller->role == 'Employer' ? 'active' : '' }}">
                            <a href="{{ route('admin.userfeedback.index', ['q' => 'Employer']) }}">Locum</a>
                        </li>
                        <li class="{{ $controller->role == 'freelancer' ? 'active' : '' }}">
                            <a href="{{ route('admin.userfeedback.index', ['q' => 'freelancer']) }}">Employer</a>
                        </li>
                    </ul>
                </div>
                <div id="fre-tab">
                        ...
                        <ul>
                            @foreach ($controller->professionslist as $profession)
                                <li style="margin-top: 25px;" {{ $controller->profession == $profession->id ? ' class=active' : '' }}>
                                    <a
                                        href="{{ route('admin.userfeedback.index', ['q' => $controller->role, 'c' => $profession->id]) }}">{{ $profession->name }}</a>
                    ...
                    <table class="table clickable table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($userFeedbacks as $key => $data)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <!--<form action="{{ route('admin.userfeedback.destroy', $data->id) }}"-->
                                        <!--    method="POST">-->
                                        <!--    @csrf-->
                                        <!--    @method('DELETE')-->
                                        <!--    <button type="submit"-->
                                        ...
                                        <!--    </button>-->
                                        <form id="delete_form_{{$data->id}}" action="{{ route('admin.userfeedback.destroy', $data->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')

                                            
                                            <button type="button" class="delete-line" data-toggle="modal" data-target="#exampleModalCenter_{{$data->id}}">
                                  ...
                                  <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                      <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLongTitle">Delete User Feedback</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                          <span aria-hidden="true">&times;</span>
                                        </button>
                                      </div>
                                      <div class="modal-body">
                                        Are you sure to delete this User Feedback   ?
                                      </div>
                                      <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                        <button type="button" onClick="deleteRole({{$data->id}})" class="btn btn-danger">Confirm</button>
                                      </div>
                                    </div>
                                  </div>
                                </div>
