        ...
        <div class="page-content" style="margin-top: -10px">
                ...
                <div class="qus-tabs financead">
                    <div class="form-group pull-left">
                        ...
                        <input type="date" id="enddate" style="padding: 5px; margin-right: 20px; width: 20%;">
                            <button onclick="getPrint();" class="btn btn-info">Export</button>
                        </div>
                    </div>
                </div>
            </div>
            <div id="fre-tab">
                <table class="table clickable table-striped table-hover">
                    ...
                    <form action="{{route('report.blockuserreport.export')}}" id="exportForm" method="post">
                    @csrf
                        <tbody class="" id="render_locam">
                            @include('admin.reports.block-partial')
                        </tbody>
                    </form>
                </table>
            ...
            <script>
                function getPrint(){
                    ...
                    var userIdArray = Array.from(userIdInputs).map(input => input.value);


                    var UserIdInputs = document.getElementsByName("user_id[]");
                    var firstnameInputs = document.getElementsByName("firstname[]");
                    var lastnameInputs = document.getElementsByName("lastname[]");
                    ...
                    for (var i = 0; i < firstnameInputs.length; i++) {
                        var entry = {
                            ...
                            firstname: firstnameInputs[i].value,
                            lastname: lastnameInputs[i].value,
                    ...
                    form.action = "{{ route('report.blockuserreport.export') }}";
                    form.method = 'POST';
                ...
                $(document).ready(function () {
                    $("#enddate").on("change", function () {
                        ...
                        if (startDateValue && endDateValue) {
                            $.ajax({
                                url: '{{route("report.blockuserreport")}}',
                                method: 'GET',
                                data: {
