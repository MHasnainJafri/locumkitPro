                ...
                <div class="white-bg contents">
                    <section class="text-left">
                        <div class="col-md-12 pad0">
                            <div class="text-capitalize finance-page-head text-center">Expense By Category</div>
                        </div>
                            ...
                            <form action="" class="add_item_form form-inline">
                                ...
                                <div class="col-md-12 pad0 mart30">
                                        ...
                                        <div class="oneyear canvas-scroll-x">
                                            <div class="mapdiv topyear">
                                                <div class="canvascover">
                                                    <div class="col-md-12">Total Expense by categories for year {{ $filter_year }}-{{ $filter_year + 1 }}</div>
                                    ...
                                    <div class="col-xs-12 col-md-4 cash_man_chart">
                                        <div class="form-group add_item_form form-inline marb0">
                                            <select name="category" class="form-control pull-right" onchange="this.form.submit()">
                                                <option value="">Choose Category </option>
                                                @foreach ($categories as $category)
                                                    <option @selected($category_filter == $category->id) value="{{ $category->id }}">{{ $category->expense }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                                ...
                                                @foreach ($expense_records as $record)
                                                    <tr>
                                                       ...
                                                       <td class="col-md-1" data-order="{{ $record->job_date }}">
                                                        ...
                                                        <td class="col-md-1"> {{ $record->expense_type->expense }} </td>
                                                        <td class="col-md-1">
...
@push('scripts')
    ...
    <script>
        ...
        var pie_category_chart_data = @json($pie_category_chart_data);
        const labels = expense_chart_data["labels"];
        const data_paid = expense_chart_data["data_paid"];
        const data_unpaid = expense_chart_data["data_unpaid"];
        ...
        //Pie supplier chart
        if (pie_category_chart_data.length == 0) {
            pie_category_chart_data = [{
                color: "#f0f0f0",
                highlight: "#fdfdfd",
        ...
        var myChart1 = new Chart(ctx1).Pie(pie_category_chart_data, options);
