...
class FeedbackQuestionController extends Controller
{
    ...
    public function index()
    {
        ...
        $allfeedback = $usersQuery->paginate(10);
    
        $categories = UserAclProfession::where('is_active','1')->get();
    ...
    public function store(Request $request)
    {
        ...
        if($request->submit == 'Save'){
            return redirect()->route('admin.feedbackquestion.index')->with('success', 'Data has been Added Successfully');
        }
        elseif($request->submit == 'Save & add new'){
            return redirect()->route('admin.feedbackquestion.create')->with('Success', 'Data has been Added Successfully');
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->submit == 'Save'){
            return redirect()->route('admin.feedbackquestion.index')->with('success', 'Data has been updated');
        }
        elseif($request->submit == 'Save & add new'){
            return redirect()->route('admin.feedbackquestion.create')->with('Success', 'Data has been updated');
        }
